...
module InlinePrettyEGraph where
...
writeDemo :: (Ord1 f, Functor f, Foldable f, Show (ENode f), Traversable f) => EGraph ana f -> IO ()
writeDemo = writeDotFile "demo.gv" . toDotGraph

toDotGraph :: (Ord1 f, Functor f, Foldable f, Show (ENode f), Traversable f) => EGraph ana f -> DotGraph Text
